...
namespace Material
{
    ...
    public partial class FrmBOM : Form
    {
        ...
        private void GetMaterialData(string sKey)
        {
            try
            {
                string SqlStr = string.Format(@"SELECT a.[ID],a.[BOM_Code],a.[BOM_Name],
                                                Convert(Varchar(100),a.Create_Time,120) Create_Time ,a.Create_By,
                                                Convert(Varchar(100),a.Modify_Time,120) Modify_Time ,a.Modify_By
                                                FROM IMOS_TA_Bom a 
                                                where Company_Code = '{0}' and Factory_Code = '{1}' and Product_Line_Code = '{2}'
                                                and (a.BOM_Code like '%{3}%' or a.BOM_Name like '%{3}%')
        ...
        private void btn_Edit_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                PlanForm.sMCode = dgvCommon.Rows[dgvCommon.CurrentRow.Index].Cells["BOM_Code"].Value.ToString();
                PlanForm.sMName = dgvCommon.Rows[dgvCommon.CurrentRow.Index].Cells["BOM_Name"].Value.ToString();
        ...
        private void btn_Del_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string sMID = dgvCommon.Rows[dgvCommon.CurrentRow.Index].Cells["BOM_Code"].Value.ToString();
                ...
                string SqlStr = string.Format(@"DELETE FROM [IMOS_TA_Bom] WHERE [BOM_Code] = '{0}'", sMID);
