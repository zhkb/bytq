...
namespace Report
{
    ...
    public partial class FrmBatchReport : Form
    {
        ...
        private void QueryBatchInfo(string DownStartTime, string DownEndTime, string sKey)
        {
            try
            {
                string SqlStr = string.Format(@"SELECT Material_Code, Material_Name, Material_Batch, Batch_Weight,CONVERT(varchar(100), [Batch_Time], 120) Batch_Time
                                                 FROM  Mixing_MaterialBatch
                                                 where CONVERT(varchar(100), [Batch_Time], 120) >= '{0}' 
                                                 and CONVERT(varchar(100), [Batch_Time], 120) <= '{1}' 
                ...
                if (sKey.Length > 0)
                {
                    SqlStr += string.Format(" and (Material_Code like '%{0}%' or Material_Name like '%{0}%') ", sKey);
                }
                //时间倒序排序
                string sOrder = " order by Batch_Time desc ";
                ...
                {
                    MasterDataSet.Tables[0].TableName = "Mixing_Batch";
            ...
            {
                SysBusinessFunction.WriteLog("物料批次信息查询异常," + ex.Message);
                SysBusinessFunction.SystemDialog(SysBusinessFunction.DialogOKMessage, "物料批次信息查询异常");
            }
        }
        ...
        private void btn_Search_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                QueryBatchInfo(PlanStartTime, PlanEndTime, sKey);//查询设置的起始日期之内的停机记录
