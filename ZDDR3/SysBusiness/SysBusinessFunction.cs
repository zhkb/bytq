...
namespace Sys.SysBusiness
{
    ...
    public struct Card
    { 
        public string Prod_Desc;   //产品型号
        public string Prod_Code;
        public string BarCode;   //产品条码
        public string Capacity;   //额定容量
        public string MaxTemperature;   //最高温度
        public string Waterproofing;   //防水等级
        public string Voltage;   //额定电压
        public string Frequency;   //额定频率
        public string Power;   //额定功率
        public string SmallPower; //标准功率
        public string FillPower; //3D功率
        public string Pressure;   //额定压力
        public string VerificationCode;   //检验码
        public string oid;   //网址
        public string makedate;//制造日期
    }
    ...
    public struct MonitorInfoInstance
    {

        public string AExceptionReminder;
        public string ABarCode;
        public string AMaterialCode;
        public string AMaterialName;
        public float AStandardWeight;
        public float ARealWeight;

    }

    public struct MonitorCurveInfo
    {
        public bool NewFlag;
        public bool SaveFlag;
        public int SerialNo;
        public string CurveTime;
        public decimal JBSpeed;//搅拌转速
        public decimal JBElectity;//搅拌电流

        public decimal FSSpeed;//分散转速
        public decimal FSEelectity;//分散电流
        public decimal Press;//压力
        public decimal Temp;//温度
    }

    public class MonitorInfo
    {
        //发泡前
        public static string BBarCode = "";
        public static string BHisBarCode = "";
        public static string BHisBarCode1 = "";
        public static string BMaterialCode = "";
        public static string BMaterialName = "";
        public static double BRealWeight = 0;
        public static double BActualWeight = 0;
        public static double BStandardWeight = 0;
        public static double BStandardError = 0;
        public static string BExceptionReminder = "";
        //发泡后
        public static string ABarCode = "";//条码
        public static string AHisBarCode = "";
        public static string AHisBarCode1 = "";
        public static string AMaterialCode = "";//门体编码
        public static string AMaterialName = "";//门体名称
        public static double ARealWeight = 0;
        public static double AActualWeight = 0;//重量
        public static double AStandardWeight = 0;//标准重量
        public static double AStandardError = 0;//误差
        public static string AExceptionReminder = "";//异常提醒
    }


    public class SysBusinessFunction
    {
        public static ArrayList MenuListInfo = new ArrayList();

        public static object StrToBinary(char v)
        {
        ...
        public static bool MasterPLCPLCConn1 = false;//设备PLC状态
        public static bool MasterPLCPLCConn2 = false;//设备PLC状态
        public static bool MasterPLCPLCConn3 = false;//设备PLC状态
        ...
        public static void GetMenuInfoData(string UserID, string UserCode)//获取用户权限信息
        {
            try
            {
                ...
                string SqlStr = string.Format(@"select a.User_ID,b.Menu_Name,b.Menu_ID,b.Menu_Code,c.Module_Code,c.Module_ID,c.Module_Name,C.Module_Source,c.Module_Form,a.Use_Flag
                                                ...
                                                and isnull(a.Use_Flag ,0)>= {4} and ISNULL(c.Module_Code,'') !=''
        ...
        public static void GetMenuInfoData1(string UserID, string UserCode)//获取用户权限信息
        {
            //try
            //{
            //    bool AdminFlag = UserCode.ToUpper() != "ADMIN";
            ...
            //    DataSet MenuDataSet = new DataSet();
            //    string SqlStr = string.Format(@"select a.User_ID,b.Parent_Name,b.Menu_ID,b.Menu_Code,b.Menu_Name,b.Menu_Source,Menu_Form,a.Use_Flag
            //                                    from Sys_UserAuthority a left join Sys_Menu b on (a.Company_Code = b.Company_Code and a.Factory_Code = b.Factory_Code and a.ProductLine_Code = b.ProductLine_Code and a.Menu_Code = b.Menu_Code)
            //                                    left join Sys_MenuParent c on (b.Company_Code = c.Company_Code and b.Factory_Code = c.Factory_Code and b.ProductLine_Code = c.ProductLine_Code and b.Parent_Code = c.Parent_Code)
            //                                    Where User_ID = {0} and a.Company_Code = '{1}' and (a.Factory_Code = '{2}' OR a.Factory_Code='0') and a.ProductLine_Code = '{3}'
            //                                    and a.Use_Flag >= {4}
            //                                    AND ISNULL(B.Menu_ID,0)>0
            //                                    Order By c.Menu_Sort,b.Parent_Name,b.Menu_Sort", UserID, BaseSystemInfo.CompanyCode, "F003", BaseSystemInfo.ProductLineCode,Convert.ToInt32(AdminFlag));
            //    MenuDataSet = DataHelper.Fill(SqlStr);
            ...
            //        for (int i = 0; i < MenuDataSet.Tables[0].Rows.Count; i++)
            //        {
            //            string CurrentParent = MenuDataSet.Tables[0].Rows[i]["Parent_Name"].ToString();

            //            if (ParentName != CurrentParent)
            //            {
            //                ParentName = CurrentParent;
            //                MenuInfo TempParentMenu = new MenuInfo();

            //                TempParentMenu.MenuType = 1;
            //                TempParentMenu.MenuID = -1;
            //                TempParentMenu.MenuCode = "";
            //                TempParentMenu.MenuName = MenuDataSet.Tables[0].Rows[i]["Parent_Name"].ToString();
            //                TempParentMenu.MenuSource = "";
            //                TempParentMenu.MenuForm = "";

            //                MenuListInfo.Add(TempParentMenu);
            //            }

            //            MenuInfo TempMenuInfo = new MenuInfo();

            //            TempMenuInfo.MenuType = 2;
            //            TempMenuInfo.MenuID = int.Parse(MenuDataSet.Tables[0].Rows[i]["Menu_ID"].ToString());
            //            TempMenuInfo.MenuCode = MenuDataSet.Tables[0].Rows[i]["Menu_Code"].ToString();
            //            TempMenuInfo.MenuName = MenuDataSet.Tables[0].Rows[i]["Menu_Name"].ToString();
            //            TempMenuInfo.MenuSource = MenuDataSet.Tables[0].Rows[i]["Menu_Source"].ToString();
            //            TempMenuInfo.MenuForm = MenuDataSet.Tables[0].Rows[i]["Menu_Form"].ToString();
            //            MenuListInfo.Add(TempMenuInfo);
            //        }
            //    }
        ...
        //报表导出 ParameterList参数列表，数据源DataSet, TableData报表中的数据源路径例如：“IMOS_PR_Material”,报表所在的路径ReportPath，导出文件的名称ExportName
        public static void ExportReport(ArrayList ParameterList, DataSet ReportDT, string TableData, string ReportPath, string ExportName)
        {
            try
            {
                if (ReportDT != null && ReportDT.Tables.Count > 0)
                {
                    ...
                    if (sfd.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        TableDataSource table = PrintReport.GetDataSource(TableData) as TableDataSource;
        ...
        // report 报表，filename 文件名， table 表格，type 类型1.预览2.打印
        public static Boolean CardPrint(FastReport.Report report,string filename, DataTable table,int type)
        {
            try
            {
                report.Load(Application.StartupPath + "\\Report\\"+ filename);
                report.RegisterData(table, "BoxBarData");
                DataBand data = report.FindObject("Data1") as DataBand;
                data.DataSource = report.GetDataSource("BoxBarData");
            ...
            {
                SysBusinessFunction.WriteLog("条码打印失败！！"+ex.Message);
                return false;
            }
        }
        public static  Card MakeCard(string code)
        {
            Card c = new Card();
            string MaterialSQL = string.Format(@"SELECT  Prod_Desc,Prod_Code,Prop_Code,Prop_Desc,Char_Desc 
                                                     FROM view_product_info 
                                                     WHERE prod_code = '{0}'", code.Substring(0, 9));
            DataTable mysqlDt = DataHelper.MySqlFill(MaterialSQL).Tables[0];
            if (mysqlDt.Rows.Count > 0)
            {
                c.Prod_Desc= mysqlDt.Rows[0]["Prod_Desc"].ToString();
                c.BarCode = code;
                c.Prod_Code = mysqlDt.Rows[0]["Prod_Code"].ToString();
                for (int i = 0; i < mysqlDt.Rows.Count; i++)
                {
                    switch (mysqlDt.Rows[i]["Prop_Code"].ToString())
                    {
                        case "PA0007":
                            c.Waterproofing = mysqlDt.Rows[i]["Char_Desc"].ToString();
                            break;
                        case "PA0003":
                            c.Voltage = mysqlDt.Rows[i]["Char_Desc"].ToString();
                            break;
                        case "PA0004":
                            c.Frequency = mysqlDt.Rows[i]["Char_Desc"].ToString();
                            break;
                        case "PA0006":
                            c.Power = mysqlDt.Rows[i]["Char_Desc"].ToString();
                            break;
                        case "PA0009":
                            c.SmallPower = mysqlDt.Rows[i]["Char_Desc"].ToString();
                            break;
                        case "PA0002":
                            c.Capacity = mysqlDt.Rows[i]["Char_Desc"].ToString();
                            break;
                        case "PA0005":
                            c.MaxTemperature = mysqlDt.Rows[i]["Char_Desc"].ToString();
                            break;
                        case "PA0008":
                            c.Pressure = mysqlDt.Rows[i]["Char_Desc"].ToString();
                            break;
                    }

                }
                c.makedate = "制造日期" + DateTime.Now.ToLocalTime().ToString("yyyyMMdd");
                string SQL = string.Format(@"SELECT Cipher,oid
                                                    FROM bns_pm_operation 
                                                    WHERE WorkUser_BarCode = '{0}'", code);
                DataTable Dt = DataHelper.MySqlFill(SQL).Tables[0];
                if (Dt.Rows.Count > 0)
                {
                    c.VerificationCode = Dt.Rows[0]["Cipher"].ToString();
                    c.oid = Dt.Rows[0]["oid"].ToString();
                }
