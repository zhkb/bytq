...
namespace Monitor
{
    public partial class StackMonitor : Form
    {
        ...
        private void InsertScan()
        {
            try
            {
                String sql = String.Format(@"Insert into  maduo_record (Prod_BarCode,Prod_Code,Prod_Name,Scan_Time,Result_Flag) values
                                                    ('{0}','{1}','{2}','{3}','{4}')", NowBarCode, NowBarCode.Substring(0, 9), "", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"), 0);
                DataHelper.MySqlFill(sql);
            }
            catch (Exception ex)
            {
