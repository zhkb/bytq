	...
	<view>
		...
		<view>
			<view class="uni-flex uni-row">
				...
				<input  class="zh-text" type="text" v-model="Part_Name" disabled  style="-webkit-flex: 1;flex: 1;"/>
			</view>
			<view class="uni-flex uni-row">
				<view  class="text">产品规格:</view>
				<input  class="zh-text" type="text" v-model="Spection" disabled  style="-webkit-flex: 1;flex: 1;"/>
			</view>
			<view class="uni-flex uni-row">
				<view  class="text">班次信息:</view>
				<input  class="zh-text" type="text" v-model="Class_Name" disabled  style="-webkit-flex: 1;flex: 1;"/>
			</view>
			<view class="uni-flex uni-row">
				<view  class="text">批次日期:</view>
				 <picker mode="date" :value="Product_Date" :start="startDate" :end="endDate" @change="bindDateChange">
				    <input   class="zh-text"  type="text" disabled v-model="Product_Date" style="-webkit-flex: 1;flex: 1;"/>
				 </picker>
			</view>
			<view class="uni-flex uni-row">
...
<script>
	...
	export default {
		...
		methods: {
			...
			SavePick(){
				getApp().globalData.PickQuaEntity.Part_Name=this.Part_Name;
				getApp().globalData.PickQuaEntity.Spection=this.Spection;
				getApp().globalData.PickQuaEntity.Class_Name=this.Class_Name;
				getApp().globalData.PickQuaEntity.Product_Date=this.Product_Date;
				getApp().globalData.PickQuaEntity.Apply_Qty=this.Apply_Qty;
				getApp().globalData.PickQuaEntity.Qty=this.Qty;
				getApp().globalData.PickQuaEntity.Location_Id=this.Location_Id;
				getApp().globalData.PickQuaEntity.Location_Name=this.Location_Name;
				console.log(getApp().globalData.PickQuaEntity)
				uni.$emit('SavePick', '1');
				uni.navigateBack({
