<template>
	<view>
		...
		<view v-for="(item, index) in maindata.StockDetailList" :key="index">
			<uni-collapse ref="add" class="warp" >
				<uni-collapse-item :title="item.Part_Name" :desc="item.Qty+''">
					...
					<uni-list >
						<uni-list-item  title="实际数量" :note="item.Qty+''" clickable @click="openMesPuop(index)"></uni-list-item>
					</uni-list>
				</uni-collapse-item>
			</uni-collapse>
		</view>
		<view class="uni-bottom-view">
			<view class="uni-flex uni-row">
				<button class="flex-item" type="primary" size="mini" @click="ReSet">重置</button>
		...
		methods: {
			...
			openMesPuop(str){
				if(this.dataType == "2"){
					...
					this.$data.AcQty = this.$data.maindata.StockDetailList[str].Qty
			...
			dialogInputConfirm(done, val) {
				if(/(^[1-9]\d*$)/.test(val)){
					this.$data.maindata.StockDetailList[this.$data.AcQtyIndex].Qty = val
					done()
				}
				else{
					uni.showLoading({
