<template>
	<view>
		...
		<view class="content">
			...
			<view v-if="current === 2">
				<view class="uni-flex uni-row">
					<input class="input-show" placeholder="请扫描产品条码" v-model="barcode"@confirm="PriBarCode" />
					<button class="btn-show" size="mini" type="primary" @click="changeState">{{opername}}</button>
					<!-- <uni-easyinput placeholder="请扫描产品条码" ></uni-easyinput> -->
				</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			changeState(){
				if(this.$data.operid==1){
					this.$data.opername = '删除';
					this.$data.operid=0;
				}else{
					this.$data.opername = '增加';
					this.$data.operid=1;
				}
			},
			PriBarCode(){
				var str = this.$data.barcode;
				console.log(str)
				console.log(this.$data.maindata.Stock_Activity_Id)
				console.log(this.$data.operid)
				uni.request({
					url: this.web_url+'/StockActivityOut/SaveScan',
					data: {
						OperTypeId:this.$data.operid,
						Stock_Activity_Id:this.$data.maindata.Stock_Activity_Id,
						BarCode:str,
						Token:token
					},
					method:"GET",
