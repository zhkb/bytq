<template>
	<scroll-view scroll-x="true"  scroll-y="true"  class="t-table" :style="{ 'border-width': border + 'px', 'border-color': borderColor }">
		<slot />
	</scroll-view>
</template>
		...
		methods: {
			...
			changeSel(){
				...
				for (let i = 0; i < this.childrens.length; i++) {
					if (this.childrens[i].checkboxData.checked && i !== 0) {
						fireArr.push(this.childrens[i].checkboxData.value - 1);
					}
				}
				this.$emit('change', {
					detail: fireArr
